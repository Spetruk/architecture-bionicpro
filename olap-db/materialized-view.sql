...
SELECT
    ...
    groupArray(muscle_group) AS muscle_usage_count_raw, -- временно
    groupUniqArray(muscle_group) AS unique_muscles_used,
    length(groupArray(muscle_group)) AS muscle_usage_count -- общее количество записей (можно уточнить логику)
